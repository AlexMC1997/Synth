...
public class Oscillator {
	...
	public Waveform generateWave() {
		return new Waveform(getWaveProfile(), getADSR());
	}
	...
	public double getSample(Waveform w, int key) {
		...
		double realVal = getHarmWaveValue(mod * w.wave.a1, w.t, w.wave.f1, key) 
					   + getHarmWaveValue(mod * w.wave.a2, w.t, w.wave.f2, key) 
					   + getHarmWaveValue(mod * w.wave.a3, w.t, w.wave.f3, key) 
					   + getHarmWaveValue(mod * w.wave.a4, w.t, w.wave.f4, key)
				;
		
		return realVal;
	}
	
	public double getHarmWaveValue(double amp, double t, double wave, int key) { 
		return amp * Math.sin(P * t * wave * Math.pow(S, key));
	}
	
	public boolean isDead(Waveform w) {
		boolean b = false;
		if (w.t + (1.0 / rate) - w.rtime >= w.adsr.release) {
			b = true;
		}
		return b;
	}
